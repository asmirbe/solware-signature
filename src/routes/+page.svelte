<script>
	...
	const debouncedSave = debounce(() => {
		if (browser) {
			saveData(false); // Pass false to indicate auto-save
		}
	}, 500);

	// Watch for store changes and trigger auto-save
	$: if (browser && $user) {
		debouncedSave();
	}
	...
	const ALLOWED_MIME_TYPES = ["image/jpeg", "image/jpg", "image/png", "image/webp", "image/bmp", "image/tiff"];
	...
	function validateFile(file) {
		...
		if (!ALLOWED_MIME_TYPES.includes(file.type)) {
			throw new Error("Format de fichier non supporté. Utilisez JPG, PNG, WEBP, BMP ou TIFF");
		}
	...
	async function uploadFunction(image, input) {
		...
		try {
			...
			const uploadData = { image: base64Data };
			...
			if (responseData.secure_url) {
				...
				throw new Error("URL sécurisée manquante dans la réponse");
			}
			...
			notifications.error(error.message || "Erreur lors du téléchargement", 3000);
			return null;
		} finally {
			// Cleanup
			if (input) {
...
<aside class="form-container">
	...
	<div class="content">
		...
		<div class="field">
			...
			<input type="tel" pattern="^9[0-9]{7}" size="10" placeholder="Numéro téléphone" id="phone" bind:value={$user.phone} />
		</div>
		<!-- Relative -->
		<div>
			<div class="separator" />
										...
										<tr>
											<td style="font-size: 12px; font-weight: bold; vertical-align: middle;">
												<a href="https://www.facebook.com/solwareauto/" target="_blank" rel="noreferrer" data-external="true">
													<img width="50%" alt="social icon facebook" style="border:none" src={socialMedia.facebook} />
												</a>
											</td>
											<td style="font-size: 12px; font-weight: bold; vertical-align: middle;">
												<a href="https://www.youtube.com/@solwaregroup" target="_blank" rel="noreferrer" data-external="true">
													<img width="50%" alt="social icon youtube" style="border:none" src={socialMedia.youtube} />
												</a>
											</td>
											<td style="font-size: 12px; font-weight: bold; vertical-align: middle;">
												<a href="https://twitter.com/solwareauto" target="_blank" rel="noreferrer" data-external="true">
													<img width="50%" alt="social icon twitter" style="border:none" src={socialMedia.twitter} />
												</a>
											</td>
											<td style="font-size: 12px; font-weight: bold; vertical-align: middle;">
												<a href={$user.linkedin || "https://www.linkedin.com/company/solware-auto/"} target="_blank" rel="noreferrer" data-external="true">
													<img width="50%" alt="social icon linkedin" style="border:none" src={socialMedia.linkedin} />
												</a>
											</td>
										</tr>
									</tbody>
