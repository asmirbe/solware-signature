<script>
	...
	async function handleSubmit(event) {
		...
		if (!$user.acceptRgpd) return notifications.info("Désolé, il est obligatoire d'accepter les condition d'utilisation.", 5000);
		if (!$user.userUniqueToken && !$user.userUniqueToken?.length > 0) return notifications.warning("Une erreur est survenu, merci de vider votre cache ou demande de l'aide.", 2000);
		if ($user.previewImage) isLoading.update((current) => !current);
		...
		if (response.ok) {
			...
			updateInlocalStore("pictureUrl", data.secure_url);
		}
...
<nav>
	<aside class="form-container" bind:this={viewport}>
		<div class="content" bind:this={contents}>
			<a href="/" class="logo">
				<svg width="1094" height="164" viewBox="0 0 1094 164" fill="none" xmlns="http://www.w3.org/2000/svg">
					<g clip-path="url(#clip0_1072_2080)">
					...
					<defs>
					<clipPath id="clip0_1072_2080">
					<rect width="1094" height="164" fill="white"/>
					</clipPath>
					</defs>
					</svg>
			...
			<div class="field">
				<label>Choix de la marque</label>
				<Dropdown opts={brandOptions} obj="brand" />
			</div>
			<div class="field">
					...
					<div class="avatar-selection">
						...
						<div class="avatar">
							{#if $user.pictureUrl || $user.previewImage}
								<img class="avatar-preview" src={$user.previewImage} alt={`Picture of ${$user.name}`} />
			...
			<div class="field">
				<div class="field -inline">
					...
					<input id="phoneMobileCheck" bind:checked={$user.mobilePhoneCheckbox} type="checkbox" class="checkbox switch" />
				</div>
			...
			<footer>
				<div class="buttons-group">
					...
					<button class="btn -primary" disabled={!$user.acceptRgpd} on:click={handleSubmit}>Créer la signature</button>
					<div class="inline-btn">
